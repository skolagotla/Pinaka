...
function generateOpenAPI(): void {
  ...
  // const specialized = getSpecializedEndpoints();
  // if (specialized && Object.keys(specialized).length > 0) {
  //   for (const [category, endpoints] of Object.entries(specialized)) {
  //     for (const [endpointName, endpoint] of Object.entries(endpoints as Record<string, any>)) {
  //       const pathKey = (endpoint as any).path;
