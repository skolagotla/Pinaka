/**
 * Generated Server Stub for Lease
 * 
...
/**
 * API Handler for Lease CRUD operations
 */
async function handler(req: NextApiRequest, res: NextApiResponse, user: UserContext) {
  ...
  const service = new LeaseService(repository);
  ...
  try {
    switch (req.method) {
      case 'GET': {
        // List Leases
        if (!req.query.id) {
          const query = leasesQuerySchema.parse(req.query);
      ...
      case 'POST': {
        const data = leasesCreateSchema.parse(req.body);
      ...
      case 'PATCH': {
        ...
        const data = leasesUpdateSchema.parse(req.body);
    ...
    console.error(`[Lease API Handler] Error:`, error);
