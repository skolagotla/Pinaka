...
// const sharp = require('sharp'); // Use dynamic import to avoid build issues
...
async function addSignatureToPDF(pdfPath, signaturePath) {
  ...
  try {
    ...
    // Use sharp to trim white space from the signature image (dynamic import)
    const sharp = await import('sharp');
    const croppedSignatureBuffer = await sharp.default(signaturePath)
    ...
    const originalMetadata = await sharp.default(signaturePath).metadata();
    const croppedMetadata = await sharp.default(croppedSignatureBuffer).metadata();
