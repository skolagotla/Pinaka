...
import { tenantRejectionSchema } from '@/lib/schemas';
import { z } from 'zod';
...
export default withAuth(async (req: NextApiRequest, res: NextApiResponse, user: UserContext) => {
  ...
  try {
    ...
    // Check permissions - landlord or pmc can reject
    ...
    // Parse request body
    const data = tenantRejectionSchema.parse(req.body);
    ...
    console.error('[Tenant Rejection v1] Error:', error);
    return res.status(500).json({
      error: error instanceof Error ? error.message : 'Failed to reject tenant',
    });
  }
