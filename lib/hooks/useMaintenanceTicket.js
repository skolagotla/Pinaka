...
export function useMaintenanceTicket(userRole = 'landlord') {
  ...
  const openTicket = useCallback(async (request) => {
    ...
    try {
      await fetch(`/api/v1/maintenance/${request.id}/mark-viewed`, {
        method: 'POST',
  ...
  const addComment = useCallback(async (comment, authorName, authorEmail) => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/maintenance/${selectedRequest.id}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        message.error(errorData.error || errorData.message || 'Failed to add comment');
      }
    } catch (error) {
  ...
  const approveTicket = useCallback(async (authorName, authorEmail) => {
    ...
    setStatusUpdateLoading(true);
    try {
      // Use v1 API endpoint for approve
      const response = await fetch(`/api/v1/maintenance/${selectedRequest.id}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const rejectTicket = useCallback(async (authorName, authorEmail) => {
    ...
    setStatusUpdateLoading(true);
    try {
      // Use v1 API endpoint for reject
      const response = await fetch(`/api/v1/maintenance/${selectedRequest.id}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
