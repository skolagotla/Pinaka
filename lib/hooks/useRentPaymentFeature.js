...
export function useRentPaymentFeature(initialData, options = {}) {
  ...
  const columns = useMemo(() => {
      ...
      },
      {
        ...
        render: (_, record) => (
          <Space size="small">
            ...
            {userRole === 'landlord' && (
              <>
                <Tooltip title={record.balance > 0 ? "Record Payment" : "View Details"}>
                  <Button
                    type="text"
                    icon={<EditOutlined />}
                    size="small"
    ...
    return baseColumns;
  }, [userRole, receipts, apiPath, formatDateDisplay, getPropertyAddress, getTenantName, getStatusTag, getTodayUTC, isDateAfter]);
