...
import FlowbitePopconfirm from '../../apps/web-app/components/shared/FlowbitePopconfirm';
  ...
  const renderActions = (record) => {
    ...
    actions.forEach((actionConfig, index) => {
      ...
      if (confirm && type === 'delete') {
        ...
        // Other actions with confirmation use FlowbitePopconfirm
        buttons.push(
          <FlowbitePopconfirm
            key={`${type}-${index}-confirm`}
            title={confirm.title}
            description={confirm.content}
            onConfirm={async () => {
              console.log(`[useGridActions] Action confirmed for record:`, record?.id || record?.email || record);
              try {
                if (confirm.onOk) {
                  await confirm.onOk(record);
                } else {
                ...
                console.error(`[useGridActions] Error in action handler:`, error);
              }
            }}
            okText="Yes"
            cancelText="No"
    ...
    const gapClass = spacing === 'large' ? 'gap-4' : spacing === 'middle' ? 'gap-3' : 'gap-2';
