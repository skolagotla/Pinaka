...
// Find the query engine in the pnpm structure
// lib/prisma.js is at root/lib/prisma.js, so we need to go up one level
const rootDir = path.resolve(__dirname, '..');
const possibleEnginePaths = [
  path.join(rootDir, 'node_modules/.pnpm/@prisma+client@6.18.0_prisma@6.19.0_typescript@5.9.3__typescript@5.9.3/node_modules/.prisma/client/libquery_engine-darwin-arm64.dylib.node'),
  path.join(rootDir, 'node_modules/.prisma/client/libquery_engine-darwin-arm64.dylib.node'),
  // Also try finding it dynamically
  ...(function findEnginePaths() {
    ...
    try {
      ...
      if (fs.existsSync(pnpmDir)) {
        ...
        const prismaDir = entries.find(e => e.startsWith('@prisma+client@'));
        if (prismaDir) {
          const enginePath = path.join(pnpmDir, prismaDir, 'node_modules/.prisma/client/libquery_engine-darwin-arm64.dylib.node');
          if (fs.existsSync(enginePath)) {
            paths.push(enginePath);
          }
        }
      }
    } catch (e) {
